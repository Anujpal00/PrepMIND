                ...
                <div className="max-h-48 overflow-y-auto space-y-2 pr-2 text-sm">
                  {extracted.questions.slice(0, 5).map((q, i) => (
                    <div key={i} className="p-2.5 rounded border border-border bg-card" data-testid={`pdf-preview-q-${i}`}>
                      {q.passage && <div className="text-[10px] uppercase tracking-wider font-bold text-orange-600 mb-1">Passage attached</div>}
                      <div className="font-medium line-clamp-2">{i + 1}. {q.question}</div>
                      <div className="text-xs text-muted-foreground mt-1">{q.options.length} options · Ans: {String.fromCharCode(65 + q.correct_index)} · {q.topic || "Untagged"}</div>
                    </div>
                  ))}
